...
(async function() {
    try {
        ...
        fs.readFile(path.join(__dirname, 'template.html'), 'utf8', (err, data) => {
            ...
            fs.readdir(path.join(__dirname, 'components'), {withFileTypes: true}, (err,data) => {
                ...
                for(const file of data) {
                    ...
                    if(file.isFile()&&temp.includes(file.name.slice(0, file.name.indexOf('.')))) {
                        let temp1 = temp;
                        fs.readFile(path.join(__dirname, 'components', file.name), 'utf-8', (err, data) => {
                            if(err) throw (err);
                            temp1 = temp1.replace(`{{${file.name.slice(0, file.name.indexOf('.'))}}}`, data);
                            console.log(temp1);
                        })
                        
                        // outputIndex.write(temp);
                    }
        ...
        //                 stream.on('end', () => {
        //                     temp = temp.replace(`{{${file.name.slice(0, file.name.indexOf('.'))}}}`, component);
        //                     outputIndex.write(temp);
        //                 });
        //             }
        //         }
        ...
        for(const file of files) {
            try {
                if(file.isDirectory()) {
                    ...
                    await copyFile(path.join(__dirname, 'assets', file.name), path.join(__dirname, 'project-dist', 'assets', file.name), fs.constants.COPYFILE_EXCL);
                }
            } catch(err) {
                if(err.code == 'EEXIST') {
                    fs.rm(path.join(__dirname, 'project-dist', 'assets', file.name), () => {
                        copyFile(path.join(__dirname, 'assets', file.name), path.join(__dirname, 'project-dist', 'assets', file.name));
                    });
                } else {
                    console.error(err);
                }
